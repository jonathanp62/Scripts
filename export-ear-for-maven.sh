...
# @(#)export-ear-for-maven.sh	0.1.3	09/01/2015
# @(#)export-ear-for-maven.sh	0.1.2	08/26/2015
# @(#)export-ear-for-maven.sh	0.1.1	10/01/2014
#
...
# Usage:
#       export-ear-for-maven.sh <project-name> [optional-configuration-file-path]
...
echo "INFO: Exporting the sources of ${PROJECT} to ${APPLICATION_BASE}..."

mvn scm:export -DconnectionUrl=scm:svn:${SVN_PROJECTS_ROOT}/${PROJECT}/trunk/src -DexportDirectory=${APPLICATION_BASE} -Dpassword=${SVN_PASS}

if [ "$?" -ne 0 ]
then
	echo -e "${color_red_light}ERROR: Failed to export the sources.${color_off}"
	exit 1
fi
